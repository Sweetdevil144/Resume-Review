...
export default function AdminSubmissionsClient() {
  ...
  return (
    <div>
      ...
      {loading && (
        <div className="card"><div className="text-sm">Loading submissions...</div></div>
      )}
      {error && (
        <div className="card"><div className="text-sm" style={{ color: "#ef4444" }}>{error}</div></div>
      )}

      <div className="grid gap-4">
        {!loading && !error && submissions.map((s) => (
          <div key={s.id} className="card submission-card">
            <div className="submission-meta">
              <div className="submission-info">
                <h4>{s.original_name}</h4>
                <div className="meta-text">
                  By: {s.profiles?.full_name || s.profiles?.email || "Unknown User"}
                </div>
                <div className="meta-text">
            ...
            <div className="form-actions" style={{ marginBottom: "0.75rem" }}>
              {s.file_url && (
                <a className="btn" href={s.file_url} target="_blank" rel="noopener noreferrer">View PDF</a>
              )}
            </div>
            ...
            <div className="form-grid">
                ...
                <select
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      ...
                      setSubmissions((prev) => prev.map((x) => x.id === s.id ? { ...x, status: value } : x));
                    } catch (err) {
              ...
              <div className="field">
                <label className="text-sm text-muted mb-1">Score (0-100)</label>
                <input
                  className="input"
                  type="number"
                  min={0}
                  ...
                  onBlur={async (e) => {
                    ...
                    try {
                      ...
                      setSubmissions((prev) => prev.map((x) => x.id === s.id ? { ...x, score: num } : x));
                    } catch {}
                  }}
                />
              </div>
              <div className="field">
                ...
                <input
                  ...
                  onBlur={async (e) => {
                    ...
                    try {
                      await updateSubmission(s.id, { admin_notes: val as any });
                      setSubmissions((prev) => prev.map((x) => x.id === s.id ? { ...x, admin_notes: val } : x));
                    } catch {}
                  }}
                />
              </div>
