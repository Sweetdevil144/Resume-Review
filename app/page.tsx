...
export default async function Home({ searchParams }: { searchParams?: { code?: string; next?: string } }) {
  ...
  return (
    <div className="container">
      <div className="hero-section">
        ...
        <p className="hero-subtitle">Streamline your resume review process with our professional platform</p>
        <div className="grid gap-3 mt-6">
          {isAuthed ? (
            <>
              {userRole === "user" && <Link className="btn btn-primary" href="/dashboard">Go to Dashboard</Link>}
              {userRole === "admin" && <Link className="btn btn-primary" href="/admin">Go to Admin Panel</Link>}
            </>
          ) : (
